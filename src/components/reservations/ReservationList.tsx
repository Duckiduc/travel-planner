...
export function ReservationList({ tripId }: { tripId: string }) {
  ...
  async function handleDelete(id: string) {
    ...
    const res = await fetch(`/api/trips/${tripId}/reservations/${id}`, {
      method: "DELETE",
    });
  ...
  return (
    <div>
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <Badge
                      variant={typeColors[item.type] ?? "default"}
                      className="text-xs"
                    >
                      {item.type}
                    </Badge>
                  ...
                  {item.confirmRef && (
                    <p className="text-xs text-gray-400">
                      Ref: <span className="font-mono">{item.confirmRef}</span>
                    </p>
                  )}
                </div>
      ...
      {editItem && (
        <ReservationForm
          tripId={tripId}
          reservationId={editItem.id}
          initialValues={{
            type: editItem.type,
            title: editItem.title,
            provider: editItem.provider,
            confirmRef: editItem.confirmRef,
